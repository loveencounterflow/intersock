(function() {
  'use strict';
  var Intersock, Intersock_client, Intersock_server, WGUY, WGUY_time, debug, defaults, get_message_class, log, primitive_types, rpr, tabulate, to_width,
    indexOf = [].indexOf;
    ...
    send($key, $value, extra) {
      return new Promise((resolve, reject) => {
        ...
        handler = (data_ui8a) => {
          ...
          tabulate(`^${this.cfg._$from}.send/handler@^`, 'reply', d);
          return resolve(d);
        };
    ...
    serve() {
      ...
      this._ws_server.on('connection', connection = (ws) => {
        ...
        this._ws.on('message', (data_ui8a) => {
          ...
          tabulate(`^${this.cfg._$from}/on_message@1^`, 'rcvd', d);
          if (d.$key !== 'received') {
            this.send('ack', {
              $ref: d.$id // JSON.stringify new @Message 'received', d
            });
          }
          return null;
        });
